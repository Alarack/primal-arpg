    ...
    public class AllIn1Shaker : MonoBehaviour
    {
        ...
        private void ShakePosition(float shake)
        {
            transform.localPosition = new Vector3(
                maximumTranslationShake.x * (Mathf.PerlinNoise(seed, Time.time * shakeFrequency) * 2 - 1) * shake,
                maximumTranslationShake.y * (Mathf.PerlinNoise(seed + 1, Time.time * shakeFrequency) * 2 - 1) * shake,
                transform.localPosition.z//maximumTranslationShake.z * (Mathf.PerlinNoise(seed + 2, Time.time * shakeFrequency) * 2 - 1)
            ) /** shake*/;
        }
